...
@Repository
public interface TeamstatsbypositionpergameRepository extends JpaRepository<Teamstatsbypositionpergame, Integer> {

	@Transactional
	@Modifying
	@Query(value = "select * from teamstatsbypositionpergame where gameid = ?1 ;", nativeQuery = true)
	ArrayList<Teamstatsbypositionpergame> findTeamStatsByPositionPerGameByGameid(int gameid);
	...
	@Query(value = "insert into teamstatsbypositionpergame("
			...
			+ "pspg.isclosed , pspg.games, " + "cast(SUM(pspg.fieldgoalsmade) as decimal(12,2)),"
	...
	void InsertTeamstatsbypositionpergame(int categoryposition, int gameid, int teamid, int minposition,
			int maxposition);
}
