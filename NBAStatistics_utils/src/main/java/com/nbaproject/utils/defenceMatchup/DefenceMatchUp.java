	...
	public static void calculateDefenceMatchUp(int gameid) {
		...
		for (int i = 0; i < playerStatsPerGameList.size(); i++) {
			...
			for (int y = 0; y < playerStatsPerGameList.size(); y++) {
				...
				if (checker == 0 && y == playerStatsPerGameList.size() - 1) {
					for (int q = 0; q < playerStatsPerGameList.size(); q++) {

						if (playerStatsPerGameList.get(q).getTeamid() != playerStatsPerGameList.get(i).getTeamid()
								&& ((Math.min(playerStatsPerGameList.get(q).getDraftkingsposition(),
										playerStatsPerGameList.get(q).getFanduelposition()) >= Math.min(
												playerStatsPerGameList.get(i).getDraftkingsposition()-1,
												playerStatsPerGameList.get(i).getFanduelposition()-1)
										&& Math.min(playerStatsPerGameList.get(q).getDraftkingsposition(),
												playerStatsPerGameList.get(q).getFanduelposition()) <= Math.max(
														playerStatsPerGameList.get(i).getDraftkingsposition()+1,
														playerStatsPerGameList.get(i).getFanduelposition()+1))
										|| (Math.max(playerStatsPerGameList.get(q).getDraftkingsposition(),
												playerStatsPerGameList.get(q).getFanduelposition()) <= Math.max(
														playerStatsPerGameList.get(i).getDraftkingsposition()+1,
														playerStatsPerGameList.get(i).getFanduelposition()+1)
												&& Math.max(playerStatsPerGameList.get(q).getDraftkingsposition(),
							...
							DefencematchupPK newdefencematchupPK = new DefencematchupPK(
									playerStatsPerGameList.get(i).getId().getGameid(),
									playerStatsPerGameList.get(i).getId().getPlayerid(),
									playerStatsPerGameList.get(q).getId().getPlayerid());
							Defencematchup newdefencematchup = new Defencematchup();

							newdefencematchup.setId(newdefencematchupPK);
							DefencematchupServiceStaticInitializer.addDefencematchup(newdefencematchup);
							System.out.println("Save DefenceMatchUp for gameId "
									+ playerStatsPerGameList.get(i).getId().getGameid() + " and playerIds: "
									+ playerStatsPerGameList.get(i).getId().getPlayerid() + " - "
									+ playerStatsPerGameList.get(q).getId().getPlayerid()+" with +-1");
		...
		for (int i = 0; i < playerStatsPerGameListWhichNotInDefencemMatchUp.size(); i++) {
			...
			for (int y = 0; y < allPlayerStatsPerGameList.size(); y++) {
				...
				if (allPlayerStatsPerGameList.get(y).getMinutes() > 1
						&& allPlayerStatsPerGameList.get(y)
								.getTeamid() != playerStatsPerGameListWhichNotInDefencemMatchUp.get(i).getTeamid()
						&& ((Math.min(allPlayerStatsPerGameList.get(y).getDraftkingsposition(),
								allPlayerStatsPerGameList.get(y).getFanduelposition()) >= Math.min(
										playerStatsPerGameListWhichNotInDefencemMatchUp.get(i).getDraftkingsposition(),
										playerStatsPerGameListWhichNotInDefencemMatchUp.get(i).getFanduelposition())
								&& Math.min(allPlayerStatsPerGameList.get(y).getDraftkingsposition(),
										allPlayerStatsPerGameList.get(y).getFanduelposition()) <= Math.max(
												playerStatsPerGameListWhichNotInDefencemMatchUp.get(i)
														.getDraftkingsposition(),
												playerStatsPerGameListWhichNotInDefencemMatchUp.get(i)
														.getFanduelposition()))
